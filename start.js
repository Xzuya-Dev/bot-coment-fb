...
/*clear-->*/ async function clear() {
	const ryosan = await spawn("clear", [], { stdio: "inherit" });
	ryosan.on("error", (error) => {
...
async function login() {
	...
	try {
		const get = await axios.get(
			"https://mbasic.facebook.com/settings/account/?",
			{
				headers: {
					Cookie: cookies
				}
	...
	for (i = 1; i <= parseInt(jumlah); i++) {
		...
		try {
			...
			const get = await axios.get(url, {
				headers: {
					...
					"sec-ch-ua-full-version-list":
						'"Chromium";v\u003d"118.0.5993.112", "Google Chrome";v\u003d"118.0.5993.112", "Not\u003dA?Brand";v\u003d"99.0.0.0"',
			...
			try {
				...
				if (posting.statusText === "OK")
					process.stdout.write(`\r${G}Succes Send Comment${W}\n`);
			} catch {
...
async function commentMasal(url, textComment) {
	try {
		...
		for (ress of section.find("div.cy")) {
			...
			if (name && urlPost) {
				commentPromises.push(
					sendCommentWithDelay(
						urlPost,
						name,
						textComment,
						30000 * commentPromises.length
					)
				);
			}
		}
		...
		for (aLink of section.find("a")) {
			...
			if (
				textLink.includes("See more stories") ||
				textLink.includes("Lihat Berita Lain")
			) {
...
async function sendComment(url, name, textComment) {
	try {
		const kirim =
			textComment[Math.floor(Math.random() * textComment.length)];
		const cookies = await ryochan.readFile("Data/.cookie.txt");
		const get = await axios.get(url, {
			headers: {
				Cookie: cookies
